...
const EmpresaList = ({ onNavigate }) => {
  ...
  const estatisticas = {
    ...
    totalFuncoes: empresas.reduce((acc, e) => 
      acc + e.projetos?.reduce((projAcc, p) => 
        projAcc + (p.funcoes?.length || 0) + // ✅ FUNÇÕES DIRETAS DO PROJETO
        p.crs?.reduce((crAcc, cr) => 
          crAcc + (cr.funcoes?.length || 0), 0), 0), 0)
  };
  ...
  const handleConfigurarEmpresa = async (empresaId) => {
  ...
  const isEmpresaAtiva = (empresaId) => {
